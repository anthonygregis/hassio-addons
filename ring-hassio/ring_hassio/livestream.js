              ...
              if (chosenCamera) {
                for (i = 0; i < cameras.length; i++) {
                  ...
                  console.log(
                    "Checking If " +
                      cameraName +
                      " Is the same as the camera we are looking for (" +
                      chosenCamera +
                      ")"
                  );
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          ...
          console.log("Started server, listening on portss " + PORT + ".");
          return [
            4 /*yield*/,
            util_1.promisify(fs.exists)(publicOutputDirectory),
          ];
        case 2:
          if (!!_a.sent()) return [3 /*break*/, 4];
          return [
            4 /*yield*/,
            util_1.promisify(fs.mkdir)(publicOutputDirectory),
          ];
        case 3:
          _a.sent();
          _a.label = 4;
...
if (
  !("RING_REFRESH_TOKEN" in process.env) ||
  !("RING_PORT" in process.env) ||
  !("CAMERA_NAME" in process.env)
) {
  console.log(
    "Missing environment variables. Check RING_REFRESH_TOKEN, RING_PORT and CAMERA_NAME are set."
  );
  process.exit();
} else {
  startStream();
